  ...
  return (
    <section
      className={`relative overflow-hidden pt-32 pb-20 ${
        isDark ? 'bg-surface-dark hex-pattern' : 'bg-surface-raised'
      }`}
    >
      {/* Geometric decoration — no gradients */}
      {isDark && (
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <motion.div
            className="absolute -right-24 -top-24 w-80 h-80 rounded-full border border-cyan/8"
            animate={{ scale: [1, 1.08, 1] }}
            transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut' }}
          />
          <motion.div
            className="absolute -right-8 -top-8 w-48 h-48 rounded-full border border-cyan/6"
            animate={{ scale: [1.05, 0.95, 1.05] }}
            transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
          />
          <motion.div
            className="absolute -left-16 bottom-0 w-56 h-56 rounded-full border border-primary-light/10"
      ...
      <div className="container-wide relative z-10">
        ...
        {tag && (
          <motion.div
            ...
            className="mb-4"
          >
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan/10 border border-cyan/20 text-cyan text-xs font-semibold tracking-widest uppercase">
        ...
        <motion.div
          ...
          className={`mt-8 h-1 w-16 rounded-full origin-left ${isDark ? 'bg-cyan' : 'bg-primary'}`}
        />
      </div>
    </section>
  )
