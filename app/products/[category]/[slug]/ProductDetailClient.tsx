...
const docConfig: Record<DocumentType, { label: string; color: string }> = {
  'COA':     { label: 'Certificate of Analysis',        color: '#243774' },
        ...
        <div className="container-wide">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {product.applications.map((app, i) => (
                    ...
                    >
                      <div className="w-5 h-5 rounded-full bg-cyan/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                        <svg className="w-3 h-3 text-cyan" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
            ...
            <div className="space-y-6 lg:sticky lg:top-28 lg:self-start">
              ...
              {/* Combined action panel: specs + CTAs */}
              <motion.div variants={fadeUp} initial="hidden" animate="show"
                ...
                <Link
                  href={`/enquiry?product=${encodeURIComponent(product.name)}&cas=${product.cas}`}
                  className="block w-full text-center py-3 rounded-xl bg-cta text-white font-bold text-sm hover:bg-cta-dark active:scale-95 transition-all duration-150 mb-2.5"
                >
                  Enquire Now
                </Link>
                <Link
                    ...
                    return (
                      <div key={doc} className="flex items-center gap-3 p-3 rounded-xl border border-border hover:border-primary/20 hover:bg-primary-xlight/20 transition-all duration-200 cursor-pointer group">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-bold text-ink" style={{ color: cfg.color }}>{doc}</div>
