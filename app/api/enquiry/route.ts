...
export async function POST(req: NextRequest) {
  ...
  // 5. Enqueue both messages. Note: we DO NOT await sendMail.
  // The queue drains in the background; the user gets 202 immediately.
  try {
    ...
    // Logo read or enqueue catastrophically failed. We still report success
    // visually (user already filled the form) but log loudly.
    console.error('[enquiry] enqueue failed', err)
    return NextResponse.json(
